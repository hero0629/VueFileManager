<template>
  <ValidationObserver @submit.prevent="updateSubcategory()" ref="updateSubCategory" v-slot="{ invalid }" tag="form">
    <div class="card shadow-card">
      <FormLabel>Edit subcategory</FormLabel>
      ...
      <ValidationProvider tag="div" mode="passive" name="name" rules="required" v-slot="{ errors }">
        <AppInputText title="Subcategory name" :error="errors[0]">
          <input  v-model="documentation.name" placeholder="Subcategory name" type="text" class="focus-border-theme input-dark" :class="{ '!border-rose-600': errors[0] }" />
        </AppInputText>
      </ValidationProvider>
      <AppInputText title="Campos" style="margin-bottom: 0;"></AppInputText>
      ...
      <span v-for="(field, index) in documentation.fields" v-bind:key="index" style="display: block ruby; width: 300px;margin-bottom: 6px;">
            ...
            <SelectInput :default="field.type"
                v-model="field.type"
                :options="fieldsType"
    ...
    <div class="flex  form-group justify-end">
      <router-link  class="flex "  :to="{ name: 'SubCategories',
                            params: { id: $route.params.type_document_id } }">
...
export default {
  ...
  methods: {
    async changeName() {
      ...
      axios
          .post(this.$store.getters.api + '/admin/sub-categories/' + this.$route.params.id, {
            attributes: {
              name: this.name,
            },
            _method: 'patch',
    ...
    async updateSubcategory() {
      const isValid = await this.$refs.updateSubCategory.validate()
      ...
      this.isLoading = true
      axios
          .patch(`/api/admin/sub-categories/${this.$route.params.id}`, this.documentation)
          .then((response) => {
            ...
            this.$router.push({
              name: 'SubCategories',
              params: { id: response.data.data.attributes.type_document_id },

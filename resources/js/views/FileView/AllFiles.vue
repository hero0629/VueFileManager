    ...
    <div>
        <MobileContextMenu>
            <OptionGroup v-if="item">
                ...
                <Option @click.native="showCreateDel(item)" :title="$t('delete')" icon="trash" />
            </OptionGroup>
            <OptionGroup v-if="item">
                <Option
        ...
        <ContextMenu>
            <template v-slot:empty-select>
                <OptionGroup>
                    <OptionUpload :title="$t('upload_files')" @click.stop.native="showCreateTipo" />
...
export default {
    ...
    computed: {
        ...mapGetters(['fastPreview', 'clipboard', 'config', 'user', 'currentFolder']),
        isFolder() {
    ...
    created() {
        this.$store.dispatch('getAllFolder', {page: 1, id:this.$route.params.id})
        ...
        events.$on('action:confirmed', (data) => {
            if (data.operation === 'delete-item-request') {
                this.$deleteFileOrFolder(data.itemInfo)
            }
    ...
    methods: {
        ...
        showCreateDel(item) {
            ...
            events.$emit('confirm:open', confirmData)
        }
    }
}
</script>

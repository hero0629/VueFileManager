...
const actions = {
    ...
    createFolder: ({ commit, getters, dispatch }, folder) => {
        ...
        axios
            ...
            .then((response) => {
                ...
                if (folder.cat_empty) {
                    dispatch('moveItem', {to_item : response.data, item: folder.cat_empty})
                }
    ...
    createFolderCustom: ({ commit, getters, dispatch }, folder) => {
		let route = {
			RequestUpload: `/api/upload-request/${router.currentRoute.params.token}/create-folder-custom`,
			Public: `/api/editor/create-folder/${router.currentRoute.params.token}`,
		}[router.currentRoute.name] || '/api/create-folder-custom'
		let parent = getters.currentFolder?.data.id		
		if (typeof parent!=="undefined") {
			setTimeout(() => {
				events.$emit('alert:open', {
					title: 'Error',
					message: 'Solo se permite en carpeta raiz',
				})
			},10)
            ...
            axios
                ...
                .then(async (response) => {
                    ...
                    if (response.data.data.id) {
                        ...
                        if (!response.data.data.attributes.parent_id) {
                            dispatch('createFolder', {name: response.data.data.attributes.tipo_nombre, emoji: null, cat_empty: response.data})
                        }

...
const actions = {
        ...
        return new Promise ((resolve, reject) => {
            ...
            axios
                ...
                .then((response) => {
                    ...
                    // let root = response.data.root_folder!=null?response.data.root_folder.data:null
    ...
    getAllFolder: ({ commit, getters },{page, id}) => {
        return new Promise ((resolve, reject) => {
            if(page === 1)
                commit('START_LOADING_VIEW')

            console.log(`${getters.api}/browse/all-folders/${id || 'all'}${getters.sorting.URI}&page=${page}`)
            axios
                .get(`${getters.api}/browse/all-folders/${id || 'all'}${getters.sorting.URI}&page=${page}`)
                .then((response) => {
                    console.log("get All Folder", response.data)
                    commit('SET_CURRENT_FOLDER', response.data.meta.root)
                    commit('SET_PAGINATOR', response.data.meta.paginate)
                    commit('STOP_LOADING_VIEW')
                    commit('ADD_NEW_ITEMS', response.data.data)
    ...
    getRecentUploads: ({commit, getters}, args) => {
        return new Promise((resolve, reject) => {
            if (args.page === 1)
                commit('START_LOADING_VIEW')
            console.log("recent param : ", args)
            axios
                .get(`/api/filter`, {
                    params: args,
                })
                .then((response) => {
                    console.log("recent upload : ", response)
                    // commit('SET_PAGINATOR', response.data.meta.paginate)

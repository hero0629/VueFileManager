...
const FunctionHelpers = {
    install(Vue) {
        Vue.prototype.$filterInput = debounce(function (user, custom, from, to, allowEmpty = false) {
            // events.$emit('data:filter', {user, custom, from, to})
            let page = 1;
            store.dispatch('getRecentUploads', {user, custom, from, to, page})
        }, 150)
        ...
        Vue.prototype.$goToFileView = function (id) {
            ...
            let locations = {
                ...
                AllFiles: { name: 'AllFiles', params: { id: id } },
            }
        ...
        Vue.prototype.$showFilter = function (filter = undefined) {
            events.$emit('filter:show', filter)
        }

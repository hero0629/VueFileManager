    ...
    methods: {
        checkGroupInView: _.debounce(function () {
            this.files.forEach((file, index) => {
                let elDom = document.getElementById(`group-${file.data.id}`)
                let scrollDom = document.getElementById('group-box');
                let element;
                let scrollBox;
                if (elDom) {
                    element = elDom.getBoundingClientRect()
                }
                if (scrollDom) {
                    scrollBox = scrollDom.getBoundingClientRect()
                }
        ...
        getFilesForView() {
            ...
            console.log("filepreviewmedia cpt", requestedFile);

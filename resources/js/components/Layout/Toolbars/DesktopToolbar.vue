                    ...
                    <PopoverItem name="desktop-create" side="left">
                        ...
                        >
                            <OptionUpload
                                ...
                                @click.stop.native="showCreateTipo"
								:class="{
                            ...
                            />
							<Option
                                @click.native="$createFolderCustomByPopup"
                                :class="{
                                    'is-inactive': (isSharedWithMe && !canEdit) || canCreateFolder || isTeamFolderHomepage || isSharedWithMeHomepage,
                                }"
                                :title="$t('create_folder') + ' - Custom'"
                                icon="folder-plus"
                            />
                        </OptionGroup>
...
export default {
    ...
    computed: {
        ...
        canCreateFolder() {
            return !this.$isThisRoute(this.$route, ['Files', 'AllFiles', 'Public', 'TeamFolders', 'SharedWithMe'])
        },
        canShowConvertToTeamFolder() {
            return this.$isThisRoute(this.$route, ['Files', 'AllFiles', 'MySharedItems'])
        },
        canUploadInView() {
            return !this.$isThisRoute(this.$route, ['Files', 'AllFiles', 'RecentUploads', 'Public', 'TeamFolders', 'SharedWithMe'])
        },
        canUploadFolderInView() {
            return !this.$isThisRoute(this.$route, ['Files', 'AllFiles', 'Public', 'TeamFolders', 'SharedWithMe'])
        },
        canDeleteInView() {
            let routes = ['TeamFolders', 'SharedWithMe', 'RecentUploads', 'MySharedItems', 'Trash', 'Public', 'Files', 'AllFiles']
            return !this.$isThisRoute(this.$route, routes) || this.clipboard.length === 0
        },
        canMoveInView() {
            let routes = ['SharedWithMe', 'RecentUploads', 'MySharedItems', 'Public', 'Files', 'AllFiles', 'TeamFolders']
            return !this.$isThisRoute(this.$route, routes) || this.clipboard.length === 0
        },
        canShareInView() {
            let routes = ['TeamFolders', 'RecentUploads', 'MySharedItems', 'Public', 'Files', 'AllFiles']
    ...
    methods: {
        ...
        showCreateTipo() {
            ...
            // events.$emit('popup:open', 'tipo-documento')
        },
        showFilter() {
            events.$emit('popup:open', 'filter-document')
        },
    },
}
</script>
